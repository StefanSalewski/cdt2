...
proc initDelaunayTriangulation*(a, b: Vector; precision = 1e-8; vertexAllocProc: VertexAllocProc = nil): DelaunayTriangulation =
  assert(a.x != b.x and a.y != b.y)
  assert(precision <= 1e-6 and precision >= 1e-14)
  result.subdivision.vertexAllocProc = vertexAllocProc
...
proc insertPointInEdge(this: var DelaunayTriangulation; point: Vector; edge: Edge; v: Vertex = nil): Vertex =
  var p = point
  var e = edge
  ...
  if v == nil:
    ...
    this.subdivision.initVertex(v, p)
    result = v
...
proc insertPointInFace(this: var DelaunayTriangulation; p: Vector; edge: Edge; v: Vertex = nil): Vertex =
...
proc insertPoint*(this: var DelaunayTriangulation; p: Vector; v: Vertex = nil): Vertex =
  let (e, locPos) = this.locatePoint(p)
  if locPos == lpEdge:
    result = this.insertPointInEdge(p, e, v)
  elif locPos == lpFace:
    result = this.insertPointInFace(p, e, v)
  elif locPos == lpOrg:
    result = e.org
  elif locPos == lpDest:
    result = e.dest
...
iterator neightbors*(v: Vertex): Vertex {.inline.} =
  let start = v.edge
  var e = start
  while true: # while e != nil ?
    yield e.dest
