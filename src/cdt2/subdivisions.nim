...
type
  Subdivision* = object of RootObj
    vertexAllocProc*: VertexAllocProc
...
proc createVertex*(s: var Subdivision; p: Vector): Vertex =
  if s.vertexAllocProc == nil:
    result = newVertex(p, s.nextVertexID)
  else:
    result = s.vertexAllocProc()
    result.point = p
    result.id = s.nextVertexID
  s.vertices[result.id.int] = result
  result.seqPos = s.vertIDs.len
  s.vertIDs.add(result.id)

proc initVertex*(s: var Subdivision; v: Vertex; p: Vector) =
  assert v != nil
  #result = newVertex(p, s.nextVertexID)
  initVertex(v, p, s.nextVertexID)
  s.vertices[v.id.int] = v
  v.seqPos = s.vertIDs.len
  s.vertIDs.add(v.id)
